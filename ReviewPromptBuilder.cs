...
namespace DBCheckAI
{
    ...
    public class ReviewPromptBuilder
    {
        ...
        public static string BuildCodeReviewPrompt(List<ReviewFile> files)
        {
            ...
            sb.AppendLine("你是一名资深 C# 架构师，负责代码审查。以下内容为本次 Git 变更的差异（diff 格式）。");
            sb.AppendLine("- '+' 开头的行表示本次新增的代码");
            sb.AppendLine("- '-' 开头的行表示本次删除的代码");
            sb.AppendLine("- 请重点关注 '+' 新增行的代码质量");
            sb.AppendLine();
            sb.AppendLine("评估维度（每项 0-20 分，总分 100）：");
            sb.AppendLine("1. 安全漏洞（SQL注入、空catch吞异常、敏感信息硬编码、危险API调用）");
            sb.AppendLine("2. 性能问题（空循环、未执行的LINQ、重复数据库查询、大对象分配）");
            sb.AppendLine("3. 设计模式符合度（SOLID 原则、常用设计模式应用）");
            sb.AppendLine("4. 代码复用性（重复代码、DRY 原则遵循、复制粘贴）");
            sb.AppendLine("5. 架构合理性（分层清晰、依赖关系合理、耦合度、方法长度）");
            sb.AppendLine("6. 可维护性（命名规范、可读性、注释完整性、死代码、无用变量）");
            sb.AppendLine();
            ...
            sb.AppendLine("      \"category\": \"安全漏洞|性能|设计模式|代码复用|架构|可维护性\",");
            sb.AppendLine("      \"message\": \"【问题描述】`代码片段（用反引号包裹具体有问题的代码行）`\",");
            sb.AppendLine("      \"suggestion\": \"改进建议\"");
            sb.AppendLine("    }");
            sb.AppendLine("  ]");
            sb.AppendLine("}");
            sb.AppendLine();
            sb.AppendLine("严重级别说明：");
            sb.AppendLine("- critical：安全漏洞（SQL注入、空catch吞异常、敏感信息泄漏等）");
            sb.AppendLine("- warning：性能问题、重复代码、死代码、设计缺陷");
            sb.AppendLine("- info：命名规范轻微不符、代码风格建议");
            sb.AppendLine();
            sb.AppendLine("注意：");
            sb.AppendLine("- message 中必须包含具体有问题的代码片段，用反引号包裹");
            sb.AppendLine("- 如果代码没有明显问题，score 可以打 90-100，issues 数组可为空");
            sb.AppendLine("- line 如果无法确定具体行号，根据 diff 上下文尽量估算，不要填 null");
